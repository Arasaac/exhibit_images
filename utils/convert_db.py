...
def sql(cursor, query, params=None):
...
def tabla_autores():
    cur.execute("select  id_autor, autor from autores")
    autores = []
...
def imagenes():
    ...
    sql = "select id_imagen, imagen, id_tipo_imagen, id_licencia from  imagenes"
    cur.execute(sql)
    lista_imagenes = []
    keys = 'id label tipo licencia'.split()

    for im in cur.fetchall():
        d = dict(zip(keys, im))
        d['tipo'] = 'ti' + str(d['tipo'])
        d['licencia'] = 'li' + str(d['licencia'])
        lista_imagenes.append(d)

    json.dump({'items': lista_imagenes}, open('imagenes.json', 'w'))
